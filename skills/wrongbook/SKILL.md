...
- **红圈圈住答案**：高可信
- **红色扣分（如 -2、-3）紧贴单题题号/答案旁**：高可信，**该题**错
  - ⚠️ 但若 "-N" 写在**段落末尾、整页角落、靠近"得分"等汇总位置**：是**整段总扣分**，**不指示哪道单题错**——别把它当作单题判错信号
...
- `image_path`：可选。指向裁出来的图片文件路径，用于看图题/连线题/字母圈选/几何图等纯文本无法表达的题（详见 A3.5）。PDF 渲染时会自动嵌入题干下方
...
### A3.5：视觉依赖题（看图 / 连线 / 字母圈选 / 几何图）

如果错题包含图、连线、字母网格、几何图等纯文本无法表达的内容，**不要跳过**，按以下步骤处理：

1. **估算原图中题目区域的边界**：用百分比坐标（更稳健，跨分辨率不变形）。例如题目占据原图左半边的 40%-60% 高度区域 → `0,40,50,20`（x,y,w,h，单位都是百分比）
2. **用 crop.py 裁出小图**：

   ```bash
   python3 ${CLAUDE_SKILL_DIR}/scripts/crop.py \
     <原图路径> output/images/wrongbook_<时间戳>_q<编号>.png \
     --box <x>,<y>,<w>,<h> --pct
   ```
   ...
   ```bash
   python3 ${CLAUDE_SKILL_DIR}/scripts/crop.py 原图.png output/images/q3.png \
     --box 120,800,1500,400
   ```

3. **JSON 里加 `image_path` 字段**指向裁出的小图（相对当前工作目录）
4. **`content` 字段仍写题干文字**（如"圈出听到的图片"、"连线"），让学生有指引；图片会自动渲染在题干**下方**、答题区**上方**
5. 图片按内容区宽度等比缩放，最大高度 80mm，超出会缩
...
```json
{
  ...
  "source_qid": "II-2",
  "content": "听录音，圈出正确的图片：",
  "image_path": "output/images/wrongbook_20260513_140000_q3.png",
  "answer_lines": 1
}
```

**坐标估算技巧**：
- 原图 PNG 的尺寸可以读 metadata；估算时给题目区域加 5-10% 上下左右 padding，避免裁太紧
- 如果同一道题的图横跨两栏（如左侧是题号、右侧是图），整片裁出来即可，别强行精切
- 裁完先确认文件存在再写 image_path；裁失败时该题降级为纯文本，content 里加文字描述
